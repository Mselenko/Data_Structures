...
public:
	...
	void remove(const T& data, Node*& bstTree)
	{
		if (bstTree)
		{
			if (data < bstTree->data_)
			{
				remove(data, bstTree->left_);
			}
			else if (bstTree->data_ < data)
			{
				remove(data, bstTree->right_);
			}
			else
			{
				...
				else
				{
					...
					bstTree->data_ = temp->data_;

					if (prnt && prnt->left_)
					{
						remove(prnt->left_->data_, prnt->left_);
					}
					else {
						remove(bstTree->right_->data_, bstTree->right_);
					}
				}
			}
		}
